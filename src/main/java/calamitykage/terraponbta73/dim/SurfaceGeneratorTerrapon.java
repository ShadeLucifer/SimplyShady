...
public class SurfaceGeneratorTerrapon implements SurfaceGenerator {
	...
	protected SurfaceGeneratorTerrapon(World world, BasePerlinNoise<?> beachNoise, BasePerlinNoise<?> soilNoise, BasePerlinNoise<?> mainNoise, boolean generateStoneVariants) {
		this.world = world;
	...
	public SurfaceGeneratorTerrapon(World world) {
		...
		for (int z = 0; z < 16; ++z) {
			for (int x = 0; x < 16; ++x) {
				...
				for (int y = maxY; y >= minY; --y) {
					...
					if (biome != Biomes.OVERWORLD_GLACIER || fillerBlock != Blocks.BLOCK_SNOW.id()) continue;
